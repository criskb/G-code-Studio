window.GCODE_STUDIO = window.GCODE_STUDIO || {};
window.GCODE_STUDIO.NODE_DEFS = window.GCODE_STUDIO.NODE_DEFS || {};

function buildCuraMeshPayload(mesh){
  const tris = meshToTris(mesh);
  if(!tris || !tris.length) return null;
  const bounds = mesh.bounds || computeMeshBounds(tris);
  return { format:"tris", data:Array.from(tris), bounds };
}

function curaMeshSignature(payload){
  if(!payload) return "none";
  const b = payload.bounds || {};
  return [payload.data?.length || 0, b.minx, b.miny, b.minz, b.maxx, b.maxy, b.maxz].join("|");
}

function mapCuraRole(raw){
  if(!raw) return "";
  const key = String(raw).toUpperCase().trim();
  const MAP = {
    "WALL-OUTER": "wall_outer",
    "WALL-INNER": "wall_inner",
    "WALL": "walls",
    "SKIN": "top",
    "TOP": "top",
    "BOTTOM": "bottom",
    "TOP/BOTTOM": "top",
    "FILL": "infill",
    "INFILL": "infill",
    "SUPPORT": "support",
    "SUPPORT-INTERFACE": "support",
    "SKIRT": "skirt",
    "BRIM": "skirt",
    "TRAVEL": "travel",
    "PRIME-TOWER": "support",
    "PRIME_TOWER": "support",
    "SKIN-OVERHANG": "top"
  };
  return MAP[key] || key.toLowerCase();
}

function parseCuraGcodeToToolpath(gcode, profile = null){
  const lines = String(gcode || "").split(/\r?\n/);
  const layers = [];
  let currentLayer = null;
  let x = 0;
  let y = 0;
  let z = 0;
  let e = 0;
  let f = 0;
  let absolutePos = true;
  let absoluteE = true;
  let currentRole = "";
  let lengthMm = 0;
  let extrudeMm = 0;
  let timeSec = 0;

  const ensureLayer = (zVal) => {
    if (!currentLayer) {
      currentLayer = { z: zVal, moves: [] };
      layers.push(currentLayer);
      return;
    }
    if (Math.abs((currentLayer.z ?? 0) - zVal) > 1e-6 && currentLayer.moves.length) {
      currentLayer = { z: zVal, moves: [] };
      layers.push(currentLayer);
    } else {
      currentLayer.z = zVal;
    }
  };

  for (const rawLine of lines) {
    const [codePart, commentPart] = rawLine.split(";", 2);
    if (commentPart && commentPart.includes("TYPE:")) {
      const typeLabel = commentPart.split("TYPE:")[1]?.trim();
      currentRole = mapCuraRole(typeLabel);
    }
    const line = codePart.trim();
    if (!line) continue;
    const parts = line.split(/\s+/);
    const cmd = parts[0]?.toUpperCase();

    if (cmd === "G90") { absolutePos = true; continue; }
    if (cmd === "G91") { absolutePos = false; continue; }
    if (cmd === "M82") { absoluteE = true; continue; }
    if (cmd === "M83") { absoluteE = false; continue; }

    if (cmd !== "G0" && cmd !== "G1") continue;

    let nx = x;
    let ny = y;
    let nz = z;
    let ne = e;
    let hasMove = false;
    let hasE = false;

    for (let i = 1; i < parts.length; i++) {
      const token = parts[i];
      if (!token) continue;
      const key = token[0].toUpperCase();
      const value = Number(token.slice(1));
      if (!Number.isFinite(value)) continue;
      if (key === "X") { nx = absolutePos ? value : x + value; hasMove = true; }
      if (key === "Y") { ny = absolutePos ? value : y + value; hasMove = true; }
      if (key === "Z") { nz = absolutePos ? value : z + value; hasMove = true; }
      if (key === "E") { ne = absoluteE ? value : e + value; hasMove = true; hasE = true; }
      if (key === "F") { f = value; }
    }

    if (!hasMove) continue;
    ensureLayer(nz);

    const dx = nx - x;
    const dy = ny - y;
    const dz = nz - z;
    const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);
    const extruding = hasE && ne > e + 1e-6;
    const moveRole = extruding ? currentRole : "travel";
    currentLayer.moves.push({
      x: nx,
      y: ny,
      z: nz,
      kind: extruding ? "print" : "travel",
      meta: { role: moveRole, feature: moveRole }
    });

    if (dist > 0) {
      lengthMm += dist;
      if (extruding) extrudeMm += Math.max(0, ne - e);
      if (f > 0) timeSec += dist / (f / 60);
    }

    x = nx;
    y = ny;
    z = nz;
    e = ne;
  }

  const filamentDia = Number(profile?.filamentDia || profile?.filament_diameter || 0);
  const filamentArea = filamentDia > 0 ? Math.PI * Math.pow(filamentDia / 2, 2) : 0;
  const extrudedMm3 = filamentArea ? extrudeMm * filamentArea : 0;

  return {
    units: "mm",
    absoluteExtrusion: absoluteE,
    layers,
    stats: {
      length_mm: lengthMm,
      extruded_mm3: extrudedMm3,
      time_s_est: timeSec
    }
  };
}

const SCHEMA_CURAENGINE_WASM = [
  ...(typeof SCHEMA_EXTERNAL_SLICER !== "undefined" ? SCHEMA_EXTERNAL_SLICER : []),
  { kind:"group", title:"Guardrails", rows:[
    { items:[
      {key:"maxTriangles", label:"Max triangles", ui:"number", min:1000, max:5000000, step:1000, default:250000},
      {key:"timeoutMs", label:"Timeout (ms)", ui:"number", min:1000, max:300000, step:1000, default:45000},
    ]},
  ], note:"Large meshes are sliced in a worker and canceled on timeout." }
];

window.GCODE_STUDIO.NODE_DEFS["Slicer CuraEngine WASM"] = {
  title:"Slicer CuraEngine WASM",
  defaultW:380,
  defaultH:420,
  tag:"slicer",
  desc:"Slice locally with CuraEngine WASM (lazy-loaded worker).",
  inputs: [
    {name:"mesh", type:"mesh"},
    {name:"profile", type:"profile"},
    {name:"settings", type:"slicer_settings"},
    {name:"overrides", type:"json"}
  ],
  outputs: [
    {name:"toolpath", type:"toolpath"},
    {name:"path", type:"path"},
    {name:"stats", type:"json"},
    {name:"preview", type:"preview"}
  ],
  initData: ()=>({ maxTriangles:250000, timeoutMs:45000 }),
  render:(node, mount)=>{
    mount.innerHTML = "";
    const hint = document.createElement("div");
    hint.className = "hint";
    hint.innerHTML = "Slices locally via <code>/wasm/curaengine.wasm</code> using a worker.";
    mount.appendChild(hint);
    renderSchema(SCHEMA_CURAENGINE_WASM, node, mount);
  },
  evaluate:(node, ctx)=>{
    node.runtime = node.runtime || {};
    const runtime = node.runtime;

    const meshInput = ctx.getInput(node.id, "mesh");
    const mesh = meshInput?.mesh || meshInput?.out || meshInput || null;
    if(!mesh) return { toolpath:null, path:[], stats:null, preview:null };

    const profileInput = ctx.getInput(node.id, "profile");
    const profile = profileInput?.profile || profileInput || ctx.defaultProfile || null;

    const settingsInput = ctx.getInput(node.id, "settings");
    const overridesInput = ctx.getInput(node.id, "overrides");
    const baseSettings = settingsInput?.settings || settingsInput || {};
    const overrideSettings = overridesInput?.settings || overridesInput || {};
    const settings = { ...baseSettings, ...overrideSettings };

    const meshPayload = buildCuraMeshPayload(mesh);
    if(!meshPayload) return { toolpath:null, path:[], stats:null, preview:null };

    const triCount = (meshPayload.data?.length || 0) / 9;
    const maxTriangles = Math.max(1000, Number(node.data.maxTriangles || 250000));
    const timeoutMs = Math.max(1000, Number(node.data.timeoutMs || 45000));
    if(triCount > maxTriangles){
      const error = `Mesh too large for CuraEngine WASM (${Math.round(triCount)} triangles > ${maxTriangles}).`;
      runtime.error = error;
      runtime.result = null;
      return { toolpath:null, path:[], stats:{ error }, preview:null };
    }

    const payloadKey = JSON.stringify({
      mesh: curaMeshSignature(meshPayload),
      settings,
      profile
    });

    if(runtime.payloadKey !== payloadKey){
      runtime.payloadKey = payloadKey;
      runtime.result = null;
      runtime.error = null;
      runtime.sentKey = null;
    }

    if(!runtime.worker){
      runtime.worker = new Worker("/workers/curaengine-wasm.worker.js");
      runtime.worker.onmessage = (event)=>{
        const msg = event.data || {};
        if(msg.requestId !== runtime.requestId) return;
        if(runtime.timeoutHandle){
          clearTimeout(runtime.timeoutHandle);
          runtime.timeoutHandle = null;
        }
        if(msg.status === "success"){
          runtime.result = { gcode: msg.gcode, stats: msg.stats, warning: msg.warning };
          runtime.error = null;
        }else{
          runtime.result = null;
          runtime.error = msg.error || "CuraEngine WASM slicing failed.";
        }
        runtime.inflight = false;
        markDirtyAuto();
      };
      runtime.worker.onerror = (err)=>{
        runtime.error = err?.message || "CuraEngine WASM worker error.";
        runtime.result = null;
        runtime.inflight = false;
        if(runtime.timeoutHandle){
          clearTimeout(runtime.timeoutHandle);
          runtime.timeoutHandle = null;
        }
        markDirtyAuto();
      };
    }

    if(!runtime.inflight && runtime.sentKey !== payloadKey){
      runtime.inflight = true;
      runtime.sentKey = payloadKey;
      runtime.requestId = (runtime.requestId || 0) + 1;
      runtime.worker.postMessage({
        requestId: runtime.requestId,
        mesh: meshPayload,
        settings,
        profile
      });
      if(runtime.timeoutHandle){
        clearTimeout(runtime.timeoutHandle);
      }
      runtime.timeoutHandle = setTimeout(()=>{
        if(runtime.worker){
          runtime.worker.terminate();
          runtime.worker = null;
        }
        runtime.inflight = false;
        runtime.result = null;
        runtime.error = "CuraEngine WASM slicing timed out.";
        markDirtyAuto();
      }, timeoutMs);
    }

    const result = runtime.result || {};
    const stats = result.stats || (runtime.error ? { error: runtime.error } : null);
    const gcode = result.gcode || "";
    let toolpath = null;
    let preview = null;
    let path = [];

    if(gcode){
      try{
        toolpath = parseCuraGcodeToToolpath(gcode, profile);
      }catch(_){
        toolpath = null;
      }
      if(toolpath){
        const converted = toolpathToPath(toolpath, { maxMoves:200000, profile });
        path = Array.isArray(converted?.path) ? converted.path : [];
        preview = { type:"toolpath", toolpath };
        if(converted?.warning) preview.warning = converted.warning;
        if(result.warning) preview.warning = result.warning;
      }
    }

    return { toolpath, path, stats, preview };
  }
};
